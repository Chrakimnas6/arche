#!/usr/bin/env bash
# check-careful.sh — PreToolUse hook for destructive command guardrails
...
if printf '%s' "$CMD" | grep -qE 'rm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+|--recursive\s+)' 2>/dev/null; then
  ...
  for target in $RM_ARGS; do
    case "$target" in
      */node_modules|node_modules|*/dist|dist|*/__pycache__|__pycache__|*/\.cache|\.cache|*/build|build|*/coverage|coverage)
        ;; # safe target
      -*)
        ;; # flag, skip
      *)
