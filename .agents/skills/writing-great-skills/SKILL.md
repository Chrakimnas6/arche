...
|---|---|---|
| Skips or violates a rule under pressure (knows better) | Prohibition + rationalization table (its excuses, pre-refuted) + red-flag phrases to self-catch | Soft guidance (_prefer…_, _consider…_) |
| Complies, but the output has the wrong shape (bloated, verdict buried) | Positive recipe: state what the output _is_ and its parts in order | Prohibition list (_don't restate_, _never narrate_) |
...
Use these to diagnose issues the user may be having with the skill — failures _of_ the skill itself, as distinct from the **baseline failure** the skill exists to fix.
